...
Возвращает путь к ближайшему предку `<...>/src/idef0/`, под которым лежит `filePath`. `null` если такого предка нет.

Реализует **lazy walk-up** из [`spec/03-layout.md`](../../../spec/03-layout.md): идём от папки `filePath` вверх по дереву до первого сегмента `src/idef0`. Корень `src/idef0/` может быть на любой глубине в workspace.
...
DiscoveryResult {
    ...
    orphans: string[]       // .idef0 файлы под scanRoot, не попавшие ни в один проект
                            // (нет A0.*.idef0 маркера в их предках вплоть до scanRoot)
}
...
Конфликты, нарушающие структуру (например, вложенный `A0.*.idef0` внутри уже найденного проекта) — **не отбрасываются** loader'ом. Возвращаются как `ProjectDescriptor` с этими файлами, чтобы validator (`@idefy/core`) мог выдать диагностику нарушения правила 15 (запрет вложенных проектов).
...
- Все функции — асинхронные (через FS adapter), без exceptions на «нормальных» ошибках FS (file not found, permission denied) — такие случаи возвращают `null`/`false` либо пустые коллекции. Истинные exceptions — только при катастрофических FS-сбоях.
...
- На `@idefy/core` — **только типы** (`Diagnostic` не использует, разве что `ProjectDescriptor` не имеет диагностик). На практике loader не зависит от `@idefy/core` структурно — это позволяет `@idefy/core` импортировать loader-типы при необходимости в будущем без циклов.
