...
поэтому в `package.json` они **не дублируются**. `activationEvents` пуст — extension просыпается только когда пользователь открывает `.idef0` или `.idef0.ascii` файл. До этого никакой код extension не выполняется.
...
8. Вызываем `core.validate(project)` для per-project правил **4, 7–14, 17, 18, 19, 20, 21, 22**. Правило 15 (nested projects) и правило 16 (orphans) — это отдельные функции:
   - `core.diagnosticsForNestedProjects(discoveryResult.nestedProjects)` оборачивает каждый `NestedProjectMarker` от loader'а в rule-15 диагностику.
   - `core.validateOrphans(discoveryResult.orphans)` создаёт rule-16 диагностику на каждый orphan-файл.
9. Объединяем диагностики: parser errors + assembler errors + validator diagnostics + orphan diagnostics + nested-project diagnostics.
